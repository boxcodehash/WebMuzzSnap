...
El APK no puede abrirlo una wallet, porque su origen es `https://localhost`. Los botones abren `https://muzzsnap-app.vercel.app/login.html#from=apk` (se cambia con `APP_PUBLIC_URL`). El mensaje firmado lleva un nonce, una caducidad de 3 minutos y `Return: apk`. La wallet abre entonces `muzzsnap://auth?token=...`. La app recupera la dirección de esa firma y vuelve a leer el saldo. Sin Cloud Functions no hay un token de un solo uso en el servidor ni un custom token de Firebase. El nonce solo se guarda en el dispositivo que acepta el enlace, así que un enlace copiado se puede reutilizar en otro teléfono hasta que caduque. «Continue in this browser» se queda en la página pública si el esquema custom falla. WalletConnect es el acceso principal dentro del APK. El identificador público del cliente está en `www/config.public.js`. La wallet vuelve con `muzzsnap://wc`, la firma se comprueba en la app y el saldo se lee otra vez. El salto por la página pública de arriba es solo el respaldo.
